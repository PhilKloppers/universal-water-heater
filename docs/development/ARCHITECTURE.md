...
This document describes the technical architecture of the Universal Water Heater custom component for Home Assistant.

## Directory Structure

```text
custom_components/universal_water_heater/
...
│   ├── __init__.py          # Exports UniversalWaterHeaterDataUpdateCoordinator
...
│   ├── __init__.py          # Exports UniversalWaterHeaterEntity
...
- `base.py` - Main coordinator class (`UniversalWaterHeaterDataUpdateCoordinator`)
...
**Key class:** `UniversalWaterHeaterDataUpdateCoordinator` (exported from `coordinator/__init__.py`)
...
- `UniversalWaterHeaterConfigFlowHandler` (main flow)
- `UniversalWaterHeaterOptionsFlow` (options)
...
2. `UniversalWaterHeaterEntity` for common functionality
...
1. Create directory: `custom_components/universal_water_heater/<platform>/`
2. Implement `__init__.py` with `async_setup_entry()`
3. Create entity classes inheriting from platform base + `UniversalWaterHeaterEntity`
