...
- **Repository:** PhilKloppers/universal-water-heater

**Key directories:**

- `custom_components/universal_water_heater/` - Main integration code
...
- Integration logs: `custom_components.universal_water_heater: debug` in `config/configuration.yaml`
...
- Use the domain `universal_water_heater` for all DOMAIN references
- Prefix all integration-specific classes with `UniversalWaterHeater`
- Use "Universal Water Heater" as the display title
...
- Inherit from platform base + `UniversalWaterHeaterEntity`
...
- `tests/` mirrors `custom_components/universal_water_heater/` structure
