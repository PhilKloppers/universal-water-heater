...
**Directory:** `custom_components/universal_water_heater/[platform]/`
...
```python
"""[Platform] platform for Universal Water Heater."""
...
from .entity import UniversalWaterHeaterEntity
from .[entity_file] import UniversalWaterHeater[EntityName]
...
async def async_setup_entry(
    ...
    coordinator: UniversalWaterHeaterDataUpdateCoordinator = hass.data[DOMAIN][
        entry.entry_id
    ]

    async_add_entities(
        [
            UniversalWaterHeater[EntityName](coordinator, entry),
...
```python
"""[Entity description] for Universal Water Heater."""
...
class UniversalWaterHeater[EntityName](
    UniversalWaterHeaterEntity,
    [PlatformEntityClass],
):
...
Add platform to `custom_components/universal_water_heater/manifest.json`:
...
- [ ] Entity class inherits from both `UniversalWaterHeaterEntity` and platform class
...
- **Base entity:** `UniversalWaterHeaterEntity` in `entity/base.py`
- **Coordinator:** `UniversalWaterHeaterDataUpdateCoordinator`
