...
- Files: `snake_case.py`
- Classes: `PascalCase` prefixed with `UniversalWaterHeater`
...
- Inherit from both platform entity and `UniversalWaterHeaterEntity` (order matters)
...
from custom_components.universal_water_heater.sensor import async_setup_entry
...
**Config entry data:** `entry_data: UniversalWaterHeaterData = hass.data[DOMAIN][entry.entry_id]`
